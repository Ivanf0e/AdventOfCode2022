...
w = 50; wl = w-1
cl = [(1,0), (2,0), (1,1), (0,2), (1,2), (0,3)]
sqes = [[ # if we exit a square at old dir, we enter new (square, dir, flip) = sqe[square][dir]
    [(1,0,0),(2,1,0),(1,2,0),(4,3,0)], # part 1
    [(0,0,0),(1,1,0),(0,2,0),(1,3,0)],
    [(2,0,0),(4,1,0),(2,2,0),(0,3,0)],
    [(4,0,0),(5,1,0),(4,2,0),(5,3,0)],
    [(3,0,0),(0,1,0),(3,2,0),(2,3,0)],
    [(5,0,0),(3,1,0),(5,2,0),(3,3,0)]],
    [
    [(1,0,0),(2,1,0),(3,0,2),(5,0,3)], # part 2
    [(4,2,2),(2,2,3),(0,2,0),(5,3,0)],
    [(1,3,3),(4,1,0),(3,1,3),(0,3,0)],
    [(4,0,0),(5,1,0),(0,0,2),(2,0,3)],
    [(1,2,2),(5,2,3),(3,2,0),(2,3,0)],
    [(4,3,3),(1,1,0),(0,1,3),(3,3,0)]]]

sqs = [[list(mp[i+c[1]*w][c[0]*w:(c[0]+1)*w]) for i in range(w)] for c in cl] # make 6 squares
d = [(1,0),(0,1),(-1,0),(0,-1)]

def ts(t): # for drawing the map on the square
    return '>' if t==0 else 'v' if t==1 else '<' if t==2 else '^'

for sqe in sqes:
    nv = [a for a in re.split('(R)|(L)',nvi) if a] # no idea why re outputs None
    x,y,t,s = 0,0,0,0 # start at x=y=t=0 in sq0
    sqs[s][y][x] = ts(t)

    while True:
        a = int(nv.pop(0))+1
        while (a := a - 1):                     # if out of moves -> take turn
            xp, yp, tp, sp = x + d[t][0], y + d[t][1], t, s # use dummy variable, so if invalid move we do not update
            if xp >= w or yp >= w or xp < 0 or yp < 0: # exiting the square
                sp, tp, dt = sqe[sp][tp]        # new square, new direction and direction change
                if tp in [0,2]:                 # enter from left or right, so update y based on x and possible flip of direction 
                    yp = wl - xp if dt == 1 else wl - yp if dt == 2 else xp if dt == 3 else yp
                    xp = 0 if tp == 0 else wl   # reset x to edge
                else:                           # enter from top or bottom
                    xp = wl - yp if dt == 1 else wl - xp if dt == 2 else yp if dt == 3 else xp
                    yp = 0 if tp == 1 else wl            
            if sqs[sp][yp][xp] == '#':  break   # unvalid move, stop walking and take turn
            else:
                x, y, t, s = xp, yp, tp, sp     # valid move, so update position
                sqs[s][y][x] = ts(t)            # draw path on square                           
        if nv:
    ...
    xp,yp = cl[s][0]*w+x,cl[s][1]*w+y
    print(1000*(yp+1) + 4 * (xp+1) + t) # 117102, 135297

# nvi = '10R5L5R10L4R5L5' # example input
# mp = open('2022_22ex.txt').read().splitlines() # map input
# w = 4; wl = w-1
# cl = [(2,0), (0,1), (1,1), (2,1), (2,2), (3,2)]
# sqes = [[ # sq_new, t_change = sqe[sq_cur][t_cur] 
#      [(0,0,0),(3,1,0),(0,2,0),(3,3,0)], # part 1
#      [(2,0,0),(1,1,0),(3,2,0),(1,3,0)],
#      [(3,0,0),(2,1,0),(1,2,0),(2,3,0)],
#      [(1,0,0),(4,1,0),(2,2,0),(0,3,0)],
#      [(5,0,0),(0,1,0),(5,2,0),(3,3,0)],
#      [(4,0,0),(5,1,0),(4,2,0),(5,3,0)]],
#      [
#      [(5,2,2),(3,1,0),(2,1,3),(1,1,2)], # part 2
#      [(2,0,0),(4,3,2),(5,3,1),(0,1,2)],
#      [(3,0,0),(4,0,1),(1,2,0),(0,0,3)],
#      [(5,1,1),(4,1,0),(2,2,0),(0,3,0)],
#      [(5,0,0),(1,3,2),(2,1,1),(3,3,0)],
#      [(0,2,2),(1,0,1),(4,2,0),(3,2,1)]]]